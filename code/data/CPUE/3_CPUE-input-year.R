...
# WHAM input:
cpue_input = matrix(0, ncol = 2, nrow = length(mod_str_yr:mod_end_yr))
cpue_use = matrix(0, ncol = 2, nrow = length(mod_str_yr:mod_end_yr))
cpue_cv = matrix(0, ncol = 2, nrow = length(mod_str_yr:mod_end_yr))
...
data <- data79 %>% group_by(Yr) %>% summarize(pr7994_m8_2R=sum(pr_7994_m8)) %>% mutate(cv = 0.2)
data = data %>% mutate(Yr = floor(Yr)) %>% group_by(Yr) %>% summarize(pr7994_m8_2R=mean(pr7994_m8_2R)) %>% mutate(cv = 0.2)
data = data %>% arrange(Yr)
...
data = Data79
data = data %>% group_by(Yr) %>% summarise(Obs = mean(Obs), CV = mean(CV))

# Add CPUE input:
pos_row = match(data$Yr, mod_str_yr:mod_end_yr)
cpue_input[pos_row, 2] = data$Obs
cpue_use[pos_row, 2] = 1
cpue_cv[pos_row, 2] = data$CV


# -------------------------------------------------------------------------

# Save data:
save(cpue_input, file = file.path('data/processed', 'CPUE-input-year.RData'))
save(cpue_use, file = file.path('data/processed', 'CPUE-use-year.RData'))
save(cpue_cv, file = file.path('data/processed', 'CPUE-cv-year.RData'))
