...
# LL selex constant over time, LL and PSLS CPUE index ------------------------------

input_data$n_indices = 2 # number of CPUEs
# Aggregated indices:
input_data$agg_indices = cpue_input*1e+03 # Obs
input_data$index_cv = cpue_cv # Obs error
input_data$use_indices = cpue_use

# Selex index pointers: 1 = LL, 3 = PSLS
input_data$selblock_pointer_indices = matrix(rep(c(1L, 3L), each = n_years), ncol = input_data$n_indices, nrow = n_years)
...
input_selex = list(model = c('len-logistic', 
                             ...
                             'len-logistic',
                             'len-logistic'),
                   initial_pars = list(c(100, 2),
                                       c(0.001, 0.9, 0.2, 0.99, 0.65), 
                                       c(45, -3, 7.5, 4, 9, -5),
                                       c(100, 2),
                                       c(100, 6)),
                   fix_pars = list(NULL, c(1,3), c(2,3,5,6), NULL, 2),
                   n_selblocks = 5)
